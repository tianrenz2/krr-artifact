...
RAW_DATA_DIR = "../rocksdb-data/"

WORKLOAD_DICT = {
    "readrandom" : "N random reader", \
    "readseq" : "N sequential reader", \
    "readwhilewriting" : "N random reader\n + 1 writer", \
    "readwhilescanning" : "N random reader\n + 1 scanner", \
    "seekrandom" : "N Random seeker", \
}
...
def draw_subgraph(metric, workload, ax_pair, enable_label=False):
    if metric == "throughput":
        rawdata_db_filepath = osp.join(RAW_DATA_DIR, f"rocksdb-{workload}-opsps.csv")
    else:
        assert metric == "latency"
        rawdata_db_filepath = osp.join(RAW_DATA_DIR, f"rocksdb-{workload}-latency.csv")
    rawdata_df = pandas.read_csv(rawdata_db_filepath)

    throughput_data_df = pandas.DataFrame({"cores": [], "mode": [], "value": []})
    ...
    overhead_data_df = pandas.DataFrame({"cores": [], "mode": [], "value": []})
    for mode in MODE_DICT:
        if mode == "baseline":
            continue
        for num_cpu in NUMCPU_LIST:
            trialdata_df = rawdata_df[(rawdata_df["mode"] == mode) & (rawdata_df["cores"] == num_cpu)]
            this_mode_throughput = throughput_data_df[(throughput_data_df["mode"] == mode) & (throughput_data_df["cores"] == num_cpu)]["value"]
            baseline_throughput = throughput_data_df[(throughput_data_df["mode"] == "baseline") & (throughput_data_df["cores"] == num_cpu)]["value"]
            ...
            #overhead = (baseline_throughput - this_mode_throughput) / baseline_throughput
            if metric == "throughput":
                overhead = baseline_throughput / this_mode_throughput
            else:
                overhead = this_mode_throughput / baseline_throughput
            row = [num_cpu, mode, overhead]
            overhead_data_df.loc[len(overhead_data_df)] = row

    throughput_data_df.sort_values('cores', inplace=True)
    throughput_data_df['cores'] = throughput_data_df['cores'].astype(str)
    overhead_data_df.sort_values('cores', inplace=True)
    overhead_data_df['cores'] = overhead_data_df['cores'].astype(str)
    ...
    if enable_label is True:
        ...
        ax1.set_xlabel("")
        ax2.set_ylabel("")
    ax2.set_xlabel(f"Num. of Cores (==N)\n{workload_description}", fontsize=16)
...
def draw_graph(metric):
    ...
    fig, ax_pair_list = plt.subplots(2, num_plots, figsize=(3.5 * num_plots, 3.4), \
    ...
    for idx in range(num_plots):
        ...
        ax_pair = [ax1, ax2]
        if idx == 0:
            draw_subgraph(metric, workload, ax_pair, enable_label=True)
        else:
            draw_subgraph(metric, workload, ax_pair)
    ...
    filename = f"rocksdb-{metric}.pdf"
    plt.savefig(filename, bbox_inches='tight')
    print("Graph is generated as {}".format(filename))

draw_graph("throughput")
draw_graph("latency")
